...
rule plot_volcano_rwr:
    input:
        OUTPATH+"table_association_rwr.csv",
    output:
        OUTFIGURES+"volcano_rwr.{e}"
    shell:
        "pygna paint-volcano-plot {input} {output} --annotate"

rule plot_volcano_sp:
    input:
        OUTPATH+"table_association_sp.csv",
    output:
        OUTFIGURES+"volcano_sp.{e}"
    shell:
        "pygna paint-volcano-plot {input} {output} --annotate"
...
rule plot_association_rwr:
   ...
   shell:
       "pygna paint-comparison-matrix {input} {output} --rwr"
...
rule plot_association_sp:
   ...
   shell:
       "pygna paint-comparison-matrix {input} {output}"
...
rule plot_within_comparison_rwr:
    ...
    shell:
        "pygna paint-comparison-matrix {input} {output} --rwr --single-geneset"
...
rule plot_within_comparison_sp:
   ...
   shell:
       "pygna paint-comparison-matrix {input} {output} --single-geneset"
